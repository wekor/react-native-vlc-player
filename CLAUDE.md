...
- **Background black screen root cause**: the vout renders via `AVSampleBufferDisplayLayer`, iOS fails the layer on backgrounding, and upstream never flushes it (vlc@fe6e76a9 `VLCSampleBufferDisplay.m` — zero status handling). `VLCFlushFailedSampleBufferLayers` flushes it from our own view tree on foreground. Verify against sources by cloning on demand: VLCKit tag 4.0.0a20 and vlc @ fe6e76a9 (sparse: modules/video_output/apple) — git protocol works where code.videolan.org web is bot-blocked.
